...
flags.DEFINE_integer("word_emb_size",   224,    "Number of learnable "
                                                "dimensions in word "
                                                "embeddings")
...
flags.DEFINE_integer("softmax_samples", 1000,   "Number of classes to sample "
                                                "for softmax")
flags.DEFINE_float  ("max_grad_norm",   20.0,   "Gradient clipping")
flags.DEFINE_bool   ("training",        True,   "Training mode, turn off for "
                                                "testing")
flags.DEFINE_string ("optimizer",       "adam", "Optimizer to use (sgd, adam, "
                                                "adagrad, adadelta)")
flags.DEFINE_float  ("learning_rate",   1e-3,   "Optimizer initial learning "
                                                "rate")
flags.DEFINE_integer("max_epoch",       50,     "Maximum number of epochs to "
                                                "run for")
flags.DEFINE_integer("max_steps",       999999, "Maximum number of steps to "
                                                "run for")

flags.DEFINE_integer("print_every",     50,     "Print every these many steps")
flags.DEFINE_integer("save_every",      -1,     "Save every these many steps "
                                                "(0 to disable, -1 for each "
                                                "epoch)")
flags.DEFINE_bool   ("save_overwrite",  True,   "Overwrite the same file each "
                                                "time")
flags.DEFINE_integer("validate_every",  1,      "Validate every these many "
                                                "epochs (0 to disable)")
