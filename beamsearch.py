...
class BeamDecoder(object):
    def __init__(self, num_classes, batch_concat=None, stop_token=0, beam_size=7,
                 max_len=20, min_op=None, length_penalty=1.0):
        """
    ...
    def wrap_cell(self, cell):
        ...
        return BeamDecoderCellWrapper(cell, self.batch_concat, self.num_classes,
                                      ...
                                      self.min_op, self.length_penalty)
...
class BeamDecoderCellWrapper(tf.nn.rnn_cell.RNNCell):
    def __init__(self, cell, batch_concat, num_classes, max_len, stop_token=0,
                 beam_size=7, min_op=None, length_penalty=1.0):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if self.min_op is None:
            ...
            min_len = tf.cast(self.min_op, tf.float32)
        lengths = tf.reduce_sum(tf.cast(tf.not_equal(done_symbols, self.stop_token),
                                        tf.float32), 1)
        factors = tf.log(lengths / min_len) * self.length_penalty
        logprobs_done_max = tf.reduce_max(logprobs_done, 1)
        logprobs_done_max_bias = logprobs_done_max + factors

        cand_symbols = tf.select(logprobs_done_max_bias > past_cand_logprobs,
                                 done_symbols, past_cand_symbols)
        cand_logprobs = tf.select(logprobs_done_max_bias > past_cand_logprobs,
